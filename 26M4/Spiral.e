...
#define BRS_CARDIAC_QSM 14 /*Add ViewOrder for cardiac QSM, JHL 06/28/2021*/
...
@host SpiralCV
float brs_daqdeloff = 2.0 with {,,2.0,VIS,"gradient delay offset",}; /*changed daqdel based on phantom calibration on cbic GE from 5us to 2us JHL 03/24/2022*/
float brs_daqdeloff_zoom = 2.0 with {,,2.0,VIS,"zoom gradient delay offset",};  /*changed daqdel based on phantom calibration on cbic GE from 5us to 2us JHL 03/24/2022*/
...
int brs_dofracz = 0 with {0,,0,VIS,"do fractional kz encoding",}; /*delete max brs_dofracz, JHL 06/30/2021*/
...
int brs_rspskp = 2 with {0,,2,VIS,"brs prescan loop times",}; /* add rspskp to have prescan under steady-state JHL 03162021*/
...
/*Add time_wait for pulse plot, JHL 06/28/2021*/
int time_wait2 = 0;
int time_wait3 = 0;
int time_wait4 = 0;
...
/*    cvdesc(opuser7, "Spiral Type");
    ...
    cvmin(opuser8, 0);
    cvmax(opuser8, 14); /*change to 14 to add cardiac QSM type, JHL 06/28/2021*/
    piuset |= use8;
    opuser8 = brs_vieword;
    ...
    /***** Add segmented opuser variables, JHL 06/29/2021 *****/
    if (pfkz_flag)
    {
...
STATUS spiral_scancore_phase(int phaseindex)
{
    ...
    /*Add seqwait to plot pulse, JHL 06/28/2021*/
    if (time_wait4 > 0)
    {
      boffset(off_seqwait4);
      setperiod(time_wait4, &seqwait4, 0);
      startseq((short)0, (SHORT)MAY_PAUSE);
    }
    ...
    if (time_wait3 > 0)
    {
      boffset(off_seqwait3);
      setperiod(time_wait3, &seqwait3, 0);
      startseq((short)0, (SHORT)MAY_PAUSE);
    }
