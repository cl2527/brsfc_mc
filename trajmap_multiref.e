...
float tm_flip = 70.0 with {0, , 70, VIS, "flip angle in trajectory mapping",};
...
int tm_disdaqs = 5 with {, , 5, VIS, "trajectory mapping disdaqs",};
int tm_nex = 8 with {, , 8, VIS, "nex of trajectory mapping",};
int tm_spiral_delay = 1ms with {, , 1ms, VIS, "spiral starting point delay with respect to slice selection gradient",};
int tm_ns = 8 with {0,20, 8, VIS, "0: x axis; 1 y axis;",};
...
tmPG(void)
{
  ...
  if (opetl>=2)
    PosGzptm = RUP_GRD(PosReadoutWindow2toN_tm + pw_gxspiral + pw_gzptma);
  else
    PosGzptm = RUP_GRD(PosReadoutWindow_tm + pw_gxspiral + pw_gzptma);

  TRAPEZOID(ZGRAD, gzptm, PosGzptm, 5000,,loggrd);

  tm_time = RUP_GRD(tm_pos_start + 
      pw_gxrf1tm + pw_gxrf1tmd + 
      pw_gx1tma + pw_gx1tm + pw_gx1tmd + pw_gxspiral +
      IMax(2, pw_gzptma+pw_gzptm+pw_gzptmd, pw_gxspoila + pw_gxspoil + pw_gxspoild + hst_flag*hst_length*2) +
      esp*(opetl-1)) + 1ms;
...
void rotate_spiral_tm(int slab, float angle)
{
  float cphi, sphi;
  int i;
  long rotmtx[1][9];
...
STATUS zero_tm_xgrad()
{
  ...
  for(count_echo=0;count_echo<opetl;count_echo++)
  {
    ...
    if (hst_flag){
      setiampt(0, &gxhyst1, opetl+count_echo); 
      setiampt(0, &gxhyst2, opetl+count_echo);
    }
  }
  return SUCCESS;
}
STATUS zero_tm_ygrad()
{
  ...
  for(count_echo=0;count_echo<opetl;count_echo++)
  {
    ...
    if (hst_flag){
      setiampt(0, &gyhyst1, opetl+count_echo); 
      setiampt(0, &gyhyst2, opetl+count_echo);
    }

  }
  return SUCCESS;
}
STATUS set_tm_xgrad(int N)
{
  int count_echo;
  ...
  if (N>0)
  {
    for(count_echo=0;count_echo<N;count_echo++)
    {
      setiamp(ia_gxspiral, &gxspiral, opetl+count_echo);
      setiampt(ia_gxspoil, &gxspoil, opetl+count_echo);
      if (hst_flag){
        setiampt( a_hst, &gxhyst1, opetl+count_echo);
        setiampt(-a_hst, &gxhyst2, opetl+count_echo);
      }
    }
  }
  return SUCCESS;
}
STATUS set_tm_ygrad(int N)
{
  int count_echo;
  ...
  if (N>0)
  {
    for(count_echo=0;count_echo<N;count_echo++)
    {
      ...
      if (hst_flag){
        setiampt( a_hst, &gyhyst1, opetl+count_echo);
        setiampt(-a_hst, &gyhyst2, opetl+count_echo);
      }
    }
  }
  return SUCCESS;
...
STATUS tm_scancore()
{
  ...
  {
    boffset(off_tmcore);
    for (count_seg = 0; count_seg < opetl+1; count_seg++)
    {
      zero_tm_xgrad();
      zero_tm_ygrad();
      if (acqmode==1 || acqmode==4)
        set_tm_xgrad(count_seg);
      else if (acqmode==2 || acqmode==3)
        set_tm_ygrad(count_seg);
      
      for (count_nex = -tm_disdaqs; count_nex < tm_nex; count_nex++)
      {
        if (count_nex <0)
          dabslice = 0;
        else
        ...
        for (count_view = 0; count_view < tm_ns; count_view++)
        {
          ...
          {
            loaddab(&(echo2toNtm[count_seg-1]), dabslice, count_seg, dabop, dabview, acq_echo2toNtm, PSD_LOAD_DAB_ALL);
          }
